...
class SimpleViewController: UIViewController {
    ...
    @objc func buttonAction() {
        ...
        if !text.isEmpty, let identifier = Int(text) {
            ...
            if identifier > DataProvider.maxIdentifier {
                showError(message: Constants.Translations.SimpleView.Alert.Error.outOfRange)
                return
            }
            
            dataProvider.search(identifier: identifier)
    ...
    func dataReceived(errorMessage: String?, on queue: DispatchQueue?) {
        DispatchQueue.main.async {
            if let message = errorMessage {
                ...
                self.showPokemonFoundAlert(name: self.dataProvider.pokemon?.name ?? Constants.Translations.SimpleView.Alert.Error.nameNotFound)
            }
        }
    }
}
